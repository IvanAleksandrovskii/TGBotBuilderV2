...
async def notify_sender(
    bot: Bot, sender_id: int, receiver_username: str, action: str, test_name: str = None
):  # TODO: Add button  (( ? ))
    try:
        if action == "completed":
            message = (
                settings.received_tests.send_tests_notifier
                + f"{receiver_username} "
                + settings.received_tests.send_tests_notifier_completed
                + f" '{test_name}'."
            )
            ...
            message = (
                settings.received_tests.send_tests_notifier
                + f"{receiver_username} "
                + settings.received_tests.send_tests_notifier_rejected
                + f" '{test_name}'."
            )
        else:
            return
...
@router.callback_query(F.data == "view_received_tests")
async def view_received_tests(callback_query: types.CallbackQuery, state: FSMContext):
    await callback_query.answer()
    receiver_id = callback_query.from_user.id
    page = 1
    await state.update_data(current_page=page)
    await show_received_tests_page(
        callback_query, callback_query.message, receiver_id, page, state
    )


@router.callback_query(F.data == "current_page")  # TODO: Double check (( ! ))
async def current_page(callback_query: types.CallbackQuery):
    await callback_query.answer(settings.received_tests.page_number)


async def show_received_tests_page(
    callback_query,
    ...
    async with db_helper.db_session() as session:
        try:
            subquery = (
                select(
                    SentTest.sender_username,
                    func.max(SentTest.created_at).label("last_sent"),
                )
                .where(
                    SentTest.receiver_username == callback_query.from_user.username,
            ...
            total_senders_query = select(
                func.count(func.distinct(SentTest.sender_username))
            ).where(
                SentTest.receiver_id == receiver_id,
                SentTest.status.in_([TestStatus.DELIVERED, TestStatus.SENT]),
            )
                ...
                for sender in senders[i : i + 2]:
                    row.append(
                        types.InlineKeyboardButton(
                            text=sender, callback_data=f"view_sender_tests_{sender}"
                        )
                    )
                keyboard.append(row)
            ...
            keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=settings.received_tests.back_button,
...
@router.callback_query(F.data.startswith("view_sender_tests_"))
async def view_sender_tests(
    ...
    async with db_helper.db_session() as session:
        try:
            log.info(
                "Fetching tests for receiver %s from sender %s",
                receiver_username,
                sender_username,
            )
            ...
            keyboard = []
            for test in tests:
                test_info = await session.execute(
                    select(Test).where(Test.id == test.test_id)
                )
                ...
                keyboard.append(
                    [
                        types.InlineKeyboardButton(
                            text=(
                                settings.received_tests.choose_sent_test_button
                                + f" {test.test_name}"
                            ),
                            callback_data=f"start_received_test_{test.id}",
                        )
                    ]
                )
            ...
            if tests:
                keyboard.append(
                    [
                        types.InlineKeyboardButton(
                            text=(
                                settings.received_tests.reject_all_from_user_button
                                + f" {sender_username}"
                            ),
                            callback_data=f"reject_all_tests_{sender_username}",
                        )
                    ]
                )
            keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=settings.received_tests.back_to_senders_list_button,
            ...
            text_service = TextService()
            text = (
                settings.received_tests.tests_from_username_button
                + f"{sender_username}:"
            )
            if not tests:
...
@router.callback_query(F.data.startswith("reject_all_tests_"))
async def reject_all_tests(callback_query: types.CallbackQuery):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=settings.received_tests.confirm_reject_all_test_button,
                    callback_data=f"confirm_reject_all_{sender_username}",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text=settings.received_tests.cancel_reject_test_button,
                    callback_data=f"view_sender_tests_{sender_username}",
                )
            ],
        ]
    )
    ...
    async with db_helper.db_session() as session:
        try:
            text = (
                settings.received_tests.reject_all_tests_confirmation
                + f" @{sender_username}?"
            )
            ...
            await send_or_edit_message(
                callback_query.message, text, keyboard, media_url
            )
        except Exception as e:
            log.exception(e)
...
@router.callback_query(F.data.startswith("confirm_reject_all_"))
async def confirm_reject_all_tests(
...
async def reject_test(callback_query: types.CallbackQuery):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(
                    text=settings.received_tests.confirm_reject_test_button,
                    callback_data=f"confirm_reject_test_{sent_test_id}",
                )
            ],
            [
                types.InlineKeyboardButton(
                    text=settings.received_tests.cancel_reject_test_button,
                    callback_data=f"cancel_reject_test_{sent_test_id}",
                )
            ],
        ]
    )

    async with db_helper.db_session() as session:  # TODO: Upgrade to send test's media with higher priority then default (( ! ))
        try:
...
@router.callback_query(F.data.startswith("confirm_reject_test_"))
async def confirm_reject_test(callback_query: types.CallbackQuery, state: FSMContext):
    await callback_query.answer()
    sent_test_id = callback_query.data.split("_")[-1]
    async with db_helper.db_session() as session:
        try:
            sent_test = await session.execute(
                select(SentTest).where(SentTest.id == sent_test_id)
            )
            ...
            if other_tests_from_sender:
                await view_sender_tests(
                    callback_query, state, sent_test.sender_username
                )
            else:
...
@router.callback_query(F.data.startswith("cancel_reject_test_"))
...
@router.callback_query(F.data.startswith("send_existing_result_"))
    ...
    async with db_helper.db_session() as session:
        try:
            ...
            test = await session.execute(
                select(Test).where(Test.id == sent_test.test_id)
            )
            ...
            if test.multi_graph_results:
                latest_results = await get_latest_category_results(
                    session, str(user.id), str(test.id)
                )

                if latest_results:
                    # Combine all category results into one message
                    combined_result_text = (
                        ""  # settings.received_tests.quiz_multi_result + "\n\n"
                    )
                    total_score = 0
                    ...
                    result_message = (
                        settings.received_tests.send_test_result_success_sent_1
                        + f" '{test.name}' "
                    ...
                    await callback_query.answer(
                        settings.received_tests.result_send_not_found_please_pass_test
                    )
                    return
            else:
                ...
                latest_result = await session.execute(
                    select(QuizResult)
                    .where(
                        QuizResult.user_id == str(user.id),
                        QuizResult.test_id == str(test.id),
                    )
                    .order_by(QuizResult.created_at.desc())
                    .limit(1)
                )
            ...
            if other_tests_from_sender:
                keyboard = types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text=settings.received_tests.back_to_recived_tests_button,
            ...
            media_url = (
                test.picture
                if test.picture
                else await TextService().get_default_media(session)
            )
            ...
            await send_or_edit_message(
                callback_query.message, result_message, keyboard, media_url
            )
...
@router.callback_query(F.data.startswith("start_received_test_"))
    ...
    async with db_helper.db_session() as session:
        try:
            ...
            latest_results = await session.execute(
                select(QuizResult)
                .where(
                    QuizResult.user_id == user_from_db.id, QuizResult.test_id == test.id
                )
                .order_by(QuizResult.created_at.desc())
                .limit(1)
            )
            ...
            if latest_result:
                if not latest_result.score is not None:  # TODO: Double check (( ! ))
                    keyboard = [
                        [
                            types.InlineKeyboardButton(
                                text=settings.received_tests.start_test_button,
                                callback_data=f"confirm_run_received_test_{test.id}",
                            )
                        ],  # sent_test_id
                    ]
                    ...
                    keyboard = [
                        ...
                        [
                            types.InlineKeyboardButton(
                                text=(
                                    settings.received_tests.send_saved_result_button
                                    + f": {latest_result.score}"
                                ),
                                callback_data=f"send_existing_result_{sent_test_id}",
                            )
                        ],
                    ]
            ...
            keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=settings.received_tests.reject_test_button,
                        callback_data=f"reject_test_{sent_test_id}",
                    )
                ]
            )
            keyboard.append(
                [
                    types.InlineKeyboardButton(
                        text=settings.received_tests.back_to_tests_list_button,
                        callback_data=f"view_sender_tests_{sent_test.sender_username}",
                    )
                ]
            )
            ...
            text = (
                settings.received_tests.start_test_confirm
                + f"\n'{test.name}'\n\n"
...
async def show_comment(
    message: types.Message, comment: str, state: FSMContext, state_group: StatesGroup
):
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
    ...
    async with db_helper.db_session() as session:
        try:
            ...
            await send_or_edit_message(message, comment_text, keyboard, media_url)
        except Exception as e:
            log.exception(e)
...
async def confirm_start_received_test(
                ...
                keyboard = types.InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            types.InlineKeyboardButton(
                                text=settings.quiz_text.quiz_continue_button,
                                callback_data="show_received_question",
                            )
                        ]
                    ]
                )
...
async def finish_received_test(message: types.Message, state: FSMContext):
    ...
    async with db_helper.db_session() as session:
        try:
            ...
            await notify_sender(
                message.bot, sent_test.sender_id, user.username, "completed", test.name
            )

            # Check for remaining tests from the same sender
            remaining_tests = await session.execute(
                select(SentTest).where(
                    SentTest.receiver_username == user.username,
                    SentTest.sender_username == sent_test.sender_username,
                    SentTest.status.in_([TestStatus.DELIVERED, TestStatus.SENT]),
                )
            )
...
@router.callback_query(
    F.data.startswith(
        (
            "start_received_test_",
            "show_received_question",
            "answ_",
            "confirm_run_received_test_",
        )
    )
)  # "quiz_back", "continue_quiz", "show_question"
